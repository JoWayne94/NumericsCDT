"""
1D linear advection dispersion test with a travelling wave solution and varying dx and CFL
...
if __name__ == '__main__':
    ...
    ngp_list = [201, 401, 601]  # testing number of cells for fair comparison with CABARET
    CFL_list = [0.4, 0.6, 0.9]

    """ Error measures """
    k = 1. # 2 * np.pi / L  # Wave number
    disp_errors_fd = np.empty((len(ngp_list), len(CFL_list)))
    disp_errors_cab = np.empty_like(disp_errors_fd)
    ...
    for cfl in range(len(CFL_list)):
        ...
        for ngp in range(len(ngp_list)):
            ...
            """ Analytical solution """
            exact_solution_fd = np.sin(k * (x[1:-1] - v[1:-1] * FINAL_TIME))
            exact_solution_cab = np.sin(k * (x_centres[1:-1] - v_centre[1:-1] * FINAL_TIME))

            # Measure phase of the wave (e.g., location of maximum or zero-crossing)
            phase_num = np.argmax(var.variables_data.values[1:-1]) * var.dx
            phase_exact = np.argmax(exact_solution_fd) * var.dx
            disp_errors_fd[ngp][cfl] = np.abs(phase_num - phase_exact) / np.abs(phase_exact)

            # Compute dispersion error based on phase deviation
            phase_num = np.argmax(msh.cell_centre_values[1:-1, 0]) * var.dx
            phase_exact = np.argmax(exact_solution_cab) * var.dx
            disp_errors_cab[ngp][cfl] = np.abs(phase_num - phase_exact) / np.abs(phase_exact)


    """ Plot convergence """
    for c in range(len(CFL_list)):

        plt.plot(np.array(ngp_list) - 1, disp_errors_fd[:, c], 'x--',
                 label='1stOrder' + str(CFL_list[c]), linewidth=lw, color=line_colours[c])

        plt.plot(np.array(ngp_list) - 1, disp_errors_cab[:, c], 'o--',
                 label='2ndOrder' + str(CFL_list[c]), linewidth=lw, markersize=ms, fillstyle='none', color=line_colours[c])
