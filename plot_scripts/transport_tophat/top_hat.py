"""
1D linear advection plots with a top-hat function
...
if __name__ == '__main__':
    ...
    """ Analytical solution """
    exact_solution_fd = np.where((0.5 <= x[1:-1]) & (x[1:-1] <= 1.5), 1., 0.)
    ...
    plt.ylim([y_bottom, y_top])
    plt.title(r'Numerical vs exact solutions (with limiter), ' + r'$T =$ ' + str(FINAL_TIME) + ' seconds')
    plt.grid()
    plt.tick_params(axis='both' , direction='in')
    plt.savefig(f'{path}/tophat.eps', dpi=1000)
    plt.show()
