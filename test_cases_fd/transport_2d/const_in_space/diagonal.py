"""
Scalar transport 2D test case with a Gaussian blob moving diagonally
...
if __name__ == '__main__':
    ...
    ic = np.where((X + center[0]) % 2. < 0.5, np.power(np.sin(2. * (X + center[0]) * np.pi), 2), 0.) * \
         np.where((Y + center[1]) % 2. < 0.5, np.power(np.sin(2. * (Y + center[1]) * np.pi), 2), 0.)
    '''======================================================'''

    """ Define driving velocity field  """
    var.variables_data.values = ic
    v = (1. / np.sqrt(2)) * np.ones((var.ny + 2, var.nx + 2, 2))
    # v = np.zeros((var.nx + 2, var.ny + 2, 2))
    # v[:, :, 0] = np.ones((var.nx + 2, var.ny + 2))
    # v[:, :, 1] = np.ones((var.nx + 2, var.ny + 2))
    mag_v = np.sqrt(np.power(v[:, :, 0], 2) + np.power(v[:, :, 1], 2))
    dt = CFL * min(var.dx, var.dy) / np.max(np.abs(mag_v))
    ...
    # Create the GIF
    with imageio.get_writer('diag.gif', mode='I', duration=0.002) as writer:
